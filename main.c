...
void PlayMusic(const wchar_t* filename) {
    ...
    // We turn the path into a short (DOS format) to avoid problems with spaces and Cyrillic characters in MCI
...
// Downloading songs from the Super_music folder
void LoadPlaylist(HWND hList) {
    ...
    if (hFind != INVALID_HANDLE_VALUE) {
        ...
        FindClose(hFind);
        SendMessageW(hList, LB_SETCURSEL, 0, 0); // Selecting the first default song
    } else {
        CreateDirectoryW(folderPath, NULL);
    }
}

// Window logic processing
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
case WM_CREATE: {
// Creating buttons
            CreateWindowW(L"BUTTON", L"⭠ Back", WS_VISIBLE | WS_CHILD, 10, 310, 70, 30, hwnd, (HMENU)ID_BTN_PREV, NULL, NULL);
            CreateWindowW(L"BUTTON", L"▶ Play", WS_VISIBLE | WS_CHILD, 85, 310, 70, 30, hwnd, (HMENU)ID_BTN_PLAY, NULL, NULL);
            CreateWindowW(L"BUTTON", L"⏸ Stop", WS_VISIBLE | WS_CHILD, 160, 310, 70, 30, hwnd, (HMENU)ID_BTN_STOP, NULL, NULL);
            CreateWindowW(L"BUTTON", L"⭢ Forward.", WS_VISIBLE | WS_CHILD, 235, 310, 70, 30, hwnd, (HMENU)ID_BTN_NEXT, NULL, NULL);
            ...
            // Defining the path to the Super_music folder next to the EXE
        ...
        case WM_COMMAND: {
            ...
            if (LOWORD(wParam) == ID_BTN_NEXT) {
                if (count > 0) {
                    // If this is the last song (count-1), switch to 0, otherwise to sel+1
            ...
            if (LOWORD(wParam) == ID_BTN_PREV) {
                if (count > 0) {
                    // If this is the first song (0), switch to the last one (count-1), otherwise to sel-1
